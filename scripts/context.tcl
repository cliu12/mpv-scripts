...
set level($j) 0
set total 0
set m($i) [menu .popupMenu -tearoff 0 ]
# set m($i) [menu .popupMenu -tearoff 0 -font Courier]
set first 1
foreach {disp rv} $::argv {
    ...
    if {$rv == ""} {
        if {$disp == "-"} {
            $m($level($j)) add separator
        } elseif {$disp == "="} {
            set i [expr $i-1]
            set j [expr $j-1]
        } else {
            $m($level($j)) add command -state disabled -label "$disp"
        }
    } elseif {$disp == "="} {
        if {$rv != ""} {
            set j [expr $j+1]
            ...
            set level($j) $i
            set total $i
            set m($i) [menu .popupMenu($i)]
            $m($level([expr $j-1])) add cascade -label "$rv" -menu $m($i)
        } else {
            
        }
    } else {
        $m($level($j)) add command -label "$disp" -command "done $rv"
    }
}
